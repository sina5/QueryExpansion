...
expandedQueryList=[]
t0 = time()
for i in range(0,countOfClusters):#countOfClusters : number of clusters
    iterCount=0
    myNewQuery=[]
    myNewQuery.clear()
    ...
    preQuery=[]   
    percentOfResultsEliminated = float(0)
    while(percentOfResultsEliminated<0.9 and iterCount<15): #Eliminate 90% of results
        percentOfResultsEliminated = 0.0
        preQuery.clear()
        ...
        for n in range(0,len(km.labels_)):
            if km.labels_[n]==i:#if it belongs to current cluster (i)
       ...
       #####Using the expanded query create a table cost/benefit/value
        table_data=[]
        table_data.clear()
        table_data = [['Keyword', 'Benefit', 'Cost', 'Value','Eliminates?']]      
        for ind in order_centroids[i, :10]:
            eliminatesResult=False
            ...
            if (otherClusterTotal!=0):
                percentOfResultsEliminated=benefit/otherClusterTotal
            rand=random.randint(0,(len(newSetOtherCluster)-1))
            
            #Now findning which of the top keywords eliminates that result.
            #Then calculate their benefit and cost.
            #Choose the one keyword with highest benefit/cost.
            lastQindex=len(myExpandedQuery)-1 #index of last keyword            
            if X[rand,lastQindex]==0: 
                eliminatesResult=True
            else:
                eliminatesResult=False
        ...
        for num in range(1,len(table_data),1): #ignore the first row (title row)
            ...
            valueElimination=table_data[num][4]
        ...
        valueElimination=table_data[maxIndex][4]
        if valueElimination=="True": ## ValueElimination
